...
import static com.solairerove.dsa.problems.P0150_EvaluateReversePolishNotation.evalRPNArray;
...
class P0150_EvaluateReversePolishNotationTest {
    ...
    void arrayLeetCodeExampleOne() {
        assertEquals(9, evalRPNArray(new String[]{"2", "1", "+", "3", "*"}));
    }

    @Test
    void arrayLeetCodeExampleTwo() {
        assertEquals(6, evalRPNArray(new String[]{"4", "13", "5", "/", "+"}));
    }

    @Test
    void arrayLeetCodeExampleThree() {
        assertEquals(22, evalRPNArray(new String[]{
                "10", "6", "9", "3", "+", "-11", "*", "/", "*", "17", "+", "5", "+"}));
    }

    @Test
    void arraySingleNumber() {
        assertEquals(42, evalRPNArray(new String[]{"42"}));
    }

    @Test
    void arraySingleNegativeNumber() {
        assertEquals(-7, evalRPNArray(new String[]{"-7"}));
    }

    @Test
    void arraySubtractionIsOrderSensitive() {
        assertEquals(-1, evalRPNArray(new String[]{"2", "3", "-"}));
    }

    @Test
    void arrayDivisionIsOrderSensitive() {
        assertEquals(0, evalRPNArray(new String[]{"3", "6", "/"}));
    }

    @Test
    void arrayDivisionTruncatesTowardZero() {
        assertEquals(-2, evalRPNArray(new String[]{"-7", "3", "/"}));
        assertEquals(2, evalRPNArray(new String[]{"7", "3", "/"}));
    }

    @Test
    void arrayMultiplicationWithNegative() {
        assertEquals(-12, evalRPNArray(new String[]{"4", "-3", "*"}));
    }

    @Test
    void arrayAllOperatorsChained() {
        assertEquals(5, evalRPNArray(new String[]{"1", "2", "+", "9", "*", "3", "/", "4", "-"}));
    }

    @Test
    void arrayDeeplyNestedExpression() {
        assertEquals(18, evalRPNArray(new String[]{"3", "4", "+", "2", "*", "7", "/", "5", "*", "8", "+"}));
    }
}
