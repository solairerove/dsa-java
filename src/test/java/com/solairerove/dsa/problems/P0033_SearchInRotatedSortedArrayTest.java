...
import static com.solairerove.dsa.problems.P0033_SearchInRotatedSortedArray.searchOnePass;
...
class P0033_SearchInRotatedSortedArrayTest {
    ...
    @Test
    void onePassLeetcodeExampleOne() {
        assertEquals(4, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 0));
    }

    @Test
    void onePassLeetcodeExampleTwo() {
        assertEquals(-1, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 3));
    }

    @Test
    void onePassLeetcodeExampleThree() {
        assertEquals(-1, searchOnePass(new int[]{1}, 0));
    }

    @Test
    void onePassSingleElementFound() {
        assertEquals(0, searchOnePass(new int[]{1}, 1));
    }

    @Test
    void onePassTwoElementsRotated() {
        assertEquals(1, searchOnePass(new int[]{3, 1}, 1));
        assertEquals(0, searchOnePass(new int[]{3, 1}, 3));
    }

    @Test
    void onePassNotRotatedFindsEveryElement() {
        int[] nums = {1, 3, 5, 7, 9, 11};
        for (int i = 0; i < nums.length; i++) {
            assertEquals(i, searchOnePass(nums, nums[i]));
        }
    }

    @Test
    void onePassTargetIsPivotValue() {
        assertEquals(4, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 0));
    }

    @Test
    void onePassTargetIsFirstElement() {
        assertEquals(0, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 4));
    }

    @Test
    void onePassTargetIsLastElement() {
        assertEquals(6, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 2));
    }

    @Test
    void onePassTargetBelowMinimum() {
        assertEquals(-1, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, -5));
    }

    @Test
    void onePassTargetAboveMaximum() {
        assertEquals(-1, searchOnePass(new int[]{4, 5, 6, 7, 0, 1, 2}, 100));
    }

    @Test
    void onePassTargetInsideGapBetweenParts() {
        assertEquals(-1, searchOnePass(new int[]{5, 6, 7, 1, 2, 3}, 4));
    }

    @Test
    void onePassNegativeValues() {
        assertEquals(1, searchOnePass(new int[]{2, -7, -3, 0}, -7));
    }

    @Test
    void onePassEveryRotationEveryTarget() {
        int[] sorted = {-4, -1, 0, 3, 8, 12, 17};
        int n = sorted.length;
        for (int k = 0; k < n; k++) {
            int[] rotated = new int[n];
            ...
            for (int i = 0; i < n; i++) {
                assertEquals(i, searchOnePass(rotated, rotated[i]));
            }
            assertEquals(-1, searchOnePass(rotated, 100));
            assertEquals(-1, searchOnePass(rotated, 1));
        }
    }

    @Test
    void onePassMatchesLinearScanOnRandomInputs() {
        Random random = new Random(42);
        for (int t = 0; t < 500; t++) {
            int n = 1 + random.nextInt(20);
            int[] sorted = random.ints(-50, 50).distinct().limit(n).sorted().toArray();
            ...
            int target = random.nextInt(101) - 50;
            assertEquals(linearScan(rotated, target), searchOnePass(rotated, target));
        }
    }
